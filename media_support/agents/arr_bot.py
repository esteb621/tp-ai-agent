...
comms_bot_tool.name = 'comms_bot_tool'
...
arr_bot = LlmAgent(
    ...
    | comms_bot_tool               | request: str                  | Tell CommsBot to notify the user         |
    ...
    STEP 3 — Call `CommsBot` with your text report as the `request` argument.
    CRITICAL: YOU MUST FORMAT YOUR FINDINGS INTO A READABLE TEXT REPORT (e.g., "media_name | Issue | Action taken | Result").
    THEN pass this TEXT REPORT as the `request` argument to `CommsBot`.
    DO NOT pass raw JSON data from your tool outputs. The user will not understand JSON.

    Once `CommsBot` returns a response, YOUR FINAL OUTPUT must be EXACTLY the text returned by `CommsBot`.
    Copy it word for word. Do not add anything. Do not summarize. Do not wrap it in JSON.
    That text IS your final response.
""",
    tools=[
        get_stuck_downloads,
        search_and_replace_release,
